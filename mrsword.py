...
def start_ssh_tunnel(port):
    ...
    try:
        ...
        print_error("İnternet bağlantısı yok! Tünel açılamaz.")
        return None, None
    ...
    def _try_tunnel(cmd, service_name):
        try:
            ...
            while time.time() - start_t < 15:
                ...
                if service_name == "pinggy" and "tcp://" in line:
                     # Örn: tcp://t.pinggy.io:12345
                     import re
    ...
    # 2. DENEME: PINGGY.IO (TCP Modu)
    print_line("Serveo yanıt vermedi. Alternatif Tünel (Pinggy.io) deneniyor...", prefix="[NETWORK]", color=Colors.YELLOW)
    # Pinggy TCP tüneli için: ssh -p 443 -R0:localhost:PORT tcp@a.pinggy.io
    # -o StrictHostKeyChecking=no ile onayı geçiyoruz
    cmd_pinggy = ["ssh", "-o", "StrictHostKeyChecking=no", "-p", "443", "-R0:localhost:" + str(port), "tcp@a.pinggy.io"]
    
    proc, addr = _try_tunnel(cmd_pinggy, "pinggy")
    if addr:
        return proc, addr
...
def start_p2p_server():
    ...
    if net_choice == '2':
        ...
        if tunnel_addr:
            ...
            # Eğer serveo ise portu ayıkla, pinggy ise zaten host:port formatında gelir
            if ":" in tunnel_addr:
            ...
            print(f"   ARKADAŞINA VERMEN GEREKEN ADRES:")
            print(f"   {Colors.WHITE}{Colors.BOLD}{host_url}:{host_port}{Colors.GREEN}")
            print(f"   (Bunu kopyala ve arkadaşına gönder, direkt yapıştırabilir)")
...
def process_command(cmd_line):
    ...
    if cmd in ['yardim', 'help']:
        ...
        if sub_choice == '1':
             ...
             print(f"\n{Colors.YELLOW}İPUCU: Arkadaşının verdiği adresi (Örn: 0.tcp.ngrok.io:14562) direkt yapıştırabilirsin.{Colors.ENDC}")
             target_input = input(f"Hedef Adres (Host:Port): ")
             ...
             if ":" in target_input:
                 ...
                 # Eğer ayrı yeten port girmek isterse diye eski usul soralım (opsiyonel)
                 p_input = input("Hedef Port (Varsayılan 5555): ")
                 if p_input.strip():
                     target_port = int(p_input)

             start_p2p_client(target_ip, target_port)
        else:
             print("İptal edildi.")
